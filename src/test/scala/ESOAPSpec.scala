...
class ESOAPSpec extends Specification {
  ...
  "ESOAP" should {
    ...
    "deserialize SOAP fault that it previously generated" in {
      val savon = ESOAP.toSOAP(
        SoapFault(
          ...
          faultactor = Some("http://uriToError.com"),
          detail = Some("erreur")
          )
        )
      ...
      fault.get.faultactor must beSome("http://uriToError.com")
      fault.get.detail must beSome("erreur")
    }
      ...
      val soapMessage = <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:typ="http://interfacedepot.acsia.gip_info_retraite.fr/types/">
          <soapenv:Body>
            <soapenv:Fault>
              ...
              <soapenv:faultactor>act</soapenv:faultactor>
              <detail>
                Message
              </detail>
            </soapenv:Fault>
    ...
    "return Some if faultactor parameter is missing" in {
      val soapMessage = <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:typ="http://interfacedepot.acsia.gip_info_retraite.fr/types/">
          <soapenv:Body>
            <soapenv:Fault>
              <soapenv:faultcode>1</soapenv:faultcode>
      ...
      val res = ESOAP.fromSOAP[SoapFault[String]](soapMessage)
      res must beSome
      res.get.faultactor must beNone
    }
    "return Some if detail parameter is missing" in {
      val soapMessage = <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:typ="http://interfacedepot.acsia.gip_info_retraite.fr/types/">
          <soapenv:Body>
            <soapenv:Fault>
              <soapenv:faultcode>1</soapenv:faultcode>
      ...
      val res = ESOAP.fromSOAP[SoapFault[String]](soapMessage)
      res must beSome
      res.get.detail must beNone
    }
    
    "serialize Soap fault with all elements" in {
      val savon = ESOAP.toSOAP(
        SoapFault(
          faultcode = SoapFault.FAULTCODE_SERVER,
          faultstring = "Super error",
      ...
      (savon \\ "faultactor").text must equalTo("http://uriToError.com")
      (savon \\ "detail").text must equalTo("erreur")
    }
    
    "serialize Soap fault without faultactor" in {
      val savon = ESOAP.toSOAP(
        SoapFault(
          faultcode = SoapFault.FAULTCODE_SERVER,
          faultstring = "Super error",
      ...
      savon \\ "faultactor" must beEmpty
    }
    
    "serialize Soap fault without faultactor and detail" in {
      val savon = ESOAP.toSOAP(
        SoapFault[String](
          faultcode = SoapFault.FAULTCODE_SERVER,
          faultstring = "Super error"
      ...
      savon \\ "detail" must beEmpty
      savon \\ "faultactor" must beEmpty
    }
    
    "deserialize Soap fault without faultactor and detail" in {
      val soapMessage = <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
          <soapenv:Body>
            <soapenv:Fault>
              <soapenv:faultcode>1</soapenv:faultcode>
              <soapenv:faultstring>2</soapenv:faultstring>
            </soapenv:Fault>
          </soapenv:Body>
        </soapenv:Envelope>
      
      val fault = ESOAP.fromSOAP[SoapFault[String]](soapMessage)
      fault.get.faultactor must beNone
      fault.get.detail must beNone
    }
    ...
    "deserialize SOAP fault generated by third party" in {
      ...
      fault.get.detail.get.param1 must equalTo("Textual informations")
      fault.get.detail.get.param2 must equalTo("More informations")
    }
